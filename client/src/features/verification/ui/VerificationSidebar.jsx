...
export const VerificationSidebar = ({ onNext, detectedAge, declaredAge = 32, canProceed = false, helperText }) => {
  const parsedDeclaredAge = Number(declaredAge);
  const parsedDetectedAge = Number(detectedAge);
  const ageMatch = Number.isFinite(parsedDetectedAge) && Number.isFinite(parsedDeclaredAge) && parsedDetectedAge >= parsedDeclaredAge;
  const ageLabel = Number.isFinite(parsedDetectedAge) ? `${parsedDetectedAge} Years` : 'Detecting...';
  const statusLabel = Number.isFinite(parsedDetectedAge) ? (ageMatch ? 'Match' : 'Mismatch') : 'Detecting age';
  const badgeClass = ageMatch ? 'bg-green-50/50 border-green-100 text-green-800' : 'bg-amber-50/50 border-amber-100 text-amber-800';
    ...
    <div className="col-span-12 lg:col-span-4 space-y-5 sm:space-y-6 lg:space-y-8">
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <div className="flex flex-col">
              ...
              <span className="text-lg font-bold text-on-surface">{parsedDeclaredAge} Years</span>
            </div>
            ...
            <div className="flex flex-col text-right">
              ...
              <span className="text-lg font-bold text-on-surface">{ageLabel}</span>
            </div>
          </div>
          <div className={`p-6 rounded-xl flex items-center gap-4 border ${ageMatch ? 'bg-green-50/50 border-green-100' : 'bg-amber-50/50 border-amber-100'}`}>
            <div className={`w-12 h-12 rounded-full flex items-center justify-center ${ageMatch ? 'bg-green-100 text-green-600' : 'bg-amber-100 text-amber-600'}`}>
              <CheckCircle2 size={24} />
            </div>
            <div>
              <h4 className="text-sm font-bold">Status: {statusLabel}</h4>
              <p className="text-xs text-on-surface-variant">User meets minimum age requirements for loan application.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="bg-surface-container rounded-xl p-5 sm:p-6 lg:p-8 space-y-6">
        ...
        <button
          onClick={onNext}
          disabled={!canProceed}
          className={`w-full text-white font-bold py-4 rounded-xl shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 ${canProceed ? 'bg-primary-gradient' : 'bg-outline-variant text-on-surface-variant cursor-not-allowed'}`}
        >
          Confirm & Continue
          <ArrowRight size={16} />
        </button>
